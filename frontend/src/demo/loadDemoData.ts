...
export async function loadDemoDatasets(options: DemoLoadOptions): Promise<void> {
  const { manifestUrl, baseUrl, datasetIds } = options;
  ...
  if (datasetIds && datasetIds.length > 0) {
    ...
    datasetIds.forEach((id) => {
      if (!knownIds.has(id)) {
        console.warn(`[Demo] Unknown dataset ID: ${id}`);
      }
    });
